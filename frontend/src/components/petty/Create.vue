      ...
      <v-row>
       ...
       <v-dialog v-model="add_acc_diag" max-width="20rem">
           <v-card style="padding:1rem">
               ...
               <v-text-field label="Margin" type="Number" step="0.01" v-model="new_acc_margin"></v-text-field>
               <v-card-actions>
                   <v-spacer></v-spacer>
...
export default {
    ...
    methods:{
        ...
        async create_account(){
            await PettyService.AddAccount(this.new_acc_code,this.new_acc_name,this.new_acc_margin);
            this.get_all_accounts();
            this.add_acc_diag = false;
        },
        async get_types(id){
            this.selected_acc = id;
            this.types = await PettyService.GetTypeByAccount(id);
        },
        async add_new_type(){
            await PettyService.AddType(this.new_type_name,this.selected_acc)
